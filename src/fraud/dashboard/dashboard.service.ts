...
export class DashboardService {
  ...
  async getStats(startDate?: string, endDate?: string) {
    ...
    const declined = all.filter(
      (t: Transaction) => t.status === 'declined',
    ).length;
    const approved = all.filter(
      (t: Transaction) => t.status === 'approved',
    ).length;
    const avgRisk =
      all.reduce((sum, t: Transaction) => sum + (t.riskScore || 0), 0) /
      (total || 1);

    return { total, approved, declined, avgRisk };
  }

  async getTopMerchants(startDate?: string, endDate?: string) {
    const where: Record<string, any> = {};
    if (startDate && endDate) {
      where.createdAt = Between(new Date(startDate), new Date(endDate));
    }
  ...
  async getRiskBreakdown(startDate?: string, endDate?: string) {
    const where: Record<string, any> = {};
    if (startDate && endDate) {
      where.createdAt = Between(new Date(startDate), new Date(endDate));
    }
